...
import type { MatchConfig, MatchSeat, MatchState, PromptState } from "../../src/types.ts";
...
run("prompt diagnostic", () => {
    ...
    for (let step = 0; step < 800; step++) {
      ...
      if (pending.length > 0) {
        ...
        if (!result.accepted) {
          ...
          reasonsByKind.get(kind)!.add(String(result.reason ?? "(no reason given)").slice(0, 140));
  ...
  for (const [k, n] of [...promptsSeen.entries()].sort((a, b) => b[1] - a[1])) {
    ...
    console.log(`  ${k.padEnd(16)} seen=${String(n).padStart(5)}  REJECTED=${String(bad).padStart(4)}`);
  }
