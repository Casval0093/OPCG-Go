...
describe("OP15-078 Mamaragan", () => {
  test("[Main] costs DON!! -2 (not -1), draws 1, and rests an opponent Character at 5000 power or less", () => {
    const engine = OnePieceTestEngine.create(
      ...
      // text carries none).
      { leaderCardId: op15Krieg001, hand: [CARD], activeDon: 2, deck: [op03Genzo046, op02Atmos003] },
      { character: [op03Merry052, op02Atmos003] },
    ...
    // Merry 3000 qualifies; Atmos 6000 does not.
    ...
    expect(
      engine
        .getView("north")
        .players.north.characters.find((card) => card?.instanceId === merryId)?.rested,
    ).toBe(true);
  });
  ...
  test("1 DON!! is not enough to pay the DON!! -2 cost", () => {
    ...
    const engine = OnePieceTestEngine.create(
      { leaderCardId: op15Krieg001, hand: [CARD], activeDon: 1, deck: [op03Genzo046, op02Atmos003] },
      { character: [op03Merry052] },
      { firstPlayer: "north", activeSeat: "south" },
    );
  ...
  test("[Counter] boosts +1000 and then draws only while you have 6 or fewer DON!! on the field", () => {
    const engine = OnePieceTestEngine.create(
      ...
      { leaderCardId: op15Krieg001, hand: [CARD], activeDon: 4, deck: [op03Genzo046, op02Atmos003] },
      { firstPlayer: "north", activeSeat: "south" },
    );
  ...
  test("[Counter] at 10 DON!! on the field boosts but does NOT draw", () => {
    ...
    const engine = OnePieceTestEngine.create(
      { leaderCardId: op15Krieg001, character: [{ card: op03Genzo046, playedOnTurn: 0 }] },
      { leaderCardId: op15Krieg001, hand: [CARD], activeDon: 10, deck: [op03Genzo046, op02Atmos003] },
      { firstPlayer: "north", activeSeat: "south" },
    );
    const attackerId = engine.findCardInZone("south", "character", op03Genzo046);
    const counterId = engine.findCardInZone("north", "hand", CARD);
