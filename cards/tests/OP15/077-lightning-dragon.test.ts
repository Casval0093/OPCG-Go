...
describe("OP15-077 Lightning Dragon", () => {
  ...
  test("the freeze is restricted to RESTED opponent Characters at 6000 power or less", () => {
    const engine = OnePieceTestEngine.create(
      ...
      {
        character: [
          { card: op03Genzo046, rested: true }, // 4000, rested   -> eligible
          { card: op02Atmos003, rested: false }, // 6000, ACTIVE  -> excluded by `state`
          { card: op02Thatch007, rested: true }, // 8000, rested  -> excluded by `power`
        ],
      },
      { firstPlayer: "north", activeSeat: "south" },
    );
    const restedSmallId = engine.findCardInZone("north", "character", op03Genzo046);
  ...
  test("a frozen Character does not become active in its controller's Refresh Phase", () => {
    ...
    expect(
      engine
        .getView("north")
        .players.north.characters.find((card) => card?.instanceId === targetId)?.rested,
    ).toBe(true);
